---
description: Methodology and fidelity notes for the impact-analysis engine.
---
...
| --- | --- | --- |
| .NET project graph | `*.csproj` `ProjectReference` / `PackageReference` | **High**, declarative and exact |
...
| .NET type references | word-boundary name matches across `.cs` files | **Medium**, can over-count or under-count |
| Angular import graph | `import ... from './...'` relative resolution | **High** for static imports |
| Angular HTTP calls | `HttpClient` `.get/.post/...` string/template URLs | **Medium**, literal and template portions only |
...
### Backend compiler-accurate analysis
...
### Frontend richer graph analysis
...
`importEdges`, `httpCalls`, `crossEdges`. You can upgrade one stage at a time
